...
function getAutocompleteSuggestions(schema, queryText, cursor, contextToken) {
    ...
    // NamedType
    if ((kind === 'NamedType' && state.prevState.kind !== 'Implements') || (kind === 'FieldDef' && step === 3)) {
        return getSuggestionsForNamedType(token, state, schema);
    }

    if (kind === 'Implements' || (kind === 'NamedType' && state.prevState.kind === 'Implements')) {
        return getSuggestionsForInterface(token, state, schema);
    }
...
function getSuggestionsForNamedType(token, state, schema) {
...
function getSuggestionsForInterface(token, state, schema) {
    var inputTypeMap = schema.getTypeMap();
    var interfaceType = _autocompleteUtils.objectValues(inputTypeMap).filter(_graphql.isInterfaceType);
    return (0, _autocompleteUtils.hintList)(token, interfaceType.map(function (type) {
        return {
            label: type.name,
            documentation: type.description,
            kind: 8
